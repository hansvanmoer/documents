...
public class EventHelperImpl implements EventHelper {
    ...
    public Mono<Void> notifyContentCreated(Content content) {
        return notify(EventConstants.CONTENT_CREATED_ROUTING_KEY, new ContentCreatedEvent(content));
    }

    @Override
    public Mono<Void> notifyRenditionCreated(Rendition rendition) {
        return notify(EventConstants.RENDITION_CREATED_ROUTING_KEY, new RenditionCreatedEvent(rendition));
    }

    @Override
    public Mono<Void> notifyRenditionDeleted(UUID renditionUuid) {
        return notify(EventConstants.RENDITION_DELETED_ROUTING_KEY, new RenditionDeletedEvent(renditionUuid));
    }

    @Override
    public Mono<Void> notifyDocumentCreated(Document document) {
        return notify(EventConstants.DOCUMENT_CREATED_ROUTING_KEY, new DocumentCreatedEvent(document));
    }

    @Override
    public Mono<Void> notifyDocumentUpdated(Document document) {
        return notify(EventConstants.DOCUMENT_UPDATED_ROUTING_KEY, new DocumentUpdatedEvent(document));
    }

    @Override
    public Mono<Void> notifyDocumentDeleted(UUID documentUuid) {
        return notify(EventConstants.DOCUMENT_DELETED_ROUTING_KEY, new DocumentDeletedEvent(documentUuid));
    }

    private Mono<Void> notify(String routingKey, Object payload) {
        return Mono.fromRunnable( () -> rabbitTemplate.convertAndSend(messageSettings.getExchangeName(), routingKey, payload))
